...
class LocalAiInferenceService {
  ...
  /// **CRITICAL FIX: Dispose models to free up RAM (prevents 8GB+ OOM crashes)**
  void dispose() {
    try {
      AppLogger.log('🧹 AI: Disposing heavy AI models from memory...');
      _translatorEnToHi?.close();
      _translatorEnToHi = null;
      
      _translatorHiToEn?.close();
      _translatorHiToEn = null;
      
      _whisper = null;
      _isWhisperInit = false;
      AppLogger.log('✅ AI: Models successfully disposed.');
    } catch (e) {
      AppLogger.log('⚠️ AI: Error while disposing models: $e');
    }
  }
}
