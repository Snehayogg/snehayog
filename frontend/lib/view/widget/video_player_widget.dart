...
class _VideoPlayerWidgetState extends State<VideoPlayerWidget>
  ...
  Future<void> _initializeController() async {
    try {
      ...
      // **NEW: Check if it's a MediaCodec error - don't retry on these**
      if (e.toString().contains('MediaCodec') ||
          e.toString().contains('ExoPlaybackException') ||
          e.toString().contains('VideoError')) {
        print(
            '🚨 VideoPlayerWidget: MediaCodec error detected - not retrying, letting VideoControllerManager handle recovery');

        setState(() {
          _errorMessage = 'Video playback error: $e';
          _isLoading = false;
        });

        // Don't retry - let VideoControllerManager handle this
        return;
      }
